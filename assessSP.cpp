  ...
  ofstream mcBio("mcBio.dat"); 
  ofstream mclnOmega("mclnOmega.dat");
  ofstream mclnq("mclnq.dat");
#include <admodel.h>
#include <contrib.h>
...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 rSeed = nyrs; // this will force unique seq each yr
  ...
  ph_msy.allocate("ph_msy");
  ph_fmsy.allocate("ph_fmsy");
  ph_omega.allocate("ph_omega");
  ph_init_B_mult.allocate("ph_init_B_mult");
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  lnMsy.allocate(ph_msy,"lnMsy");
  lnFmsy.allocate(ph_fmsy,"lnFmsy");
  pmMsy.allocate(-1,"pmMsy");
  psdMsy.allocate(-1,"psdMsy");
  pmFmsy.allocate(-1,"pmFmsy");
  psdFmsy.allocate(-1,"psdFmsy");
  rho.allocate(-1,"rho");
  lnOmega.allocate(2,nyrs-1,-2.,2.,ph_omega,"lnOmega");
  log_init_B_mult.allocate(ph_init_B_mult,"log_init_B_mult");
  MSY.allocate("MSY");
  #ifndef NO_AD_INITIALIZE
  MSY.initialize();
  #endif
  init_B_mult.allocate("init_B_mult");
  #ifndef NO_AD_INITIALIZE
  init_B_mult.initialize();
  #endif
  Fmsy.allocate("Fmsy");
  Bmsy.allocate("Bmsy");
  projExpBio.allocate("projExpBio");
  lastBt.allocate("lastBt");
...
void model_parameters::pMod(void)
{
  ...
  // Back-transform parameters.s
  MSY  = mfexp( lnMsy );
  Fmsy = mfexp( lnFmsy );
  Bmsy = MSY/Fmsy;
  init_B_mult = exp(log_init_B_mult);
