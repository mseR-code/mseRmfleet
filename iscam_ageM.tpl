...
//--   
// --29 Aug 2012: 
//--    Changes made to projection_model to reflect recent average (m_bar) in  --//
//--    forecasts and to apply sbo in the risk tables (uses longterm bo for    --//
//--    calc P(Bt +1 > 0.25Bo) )                                               --//
//--                                                                           --//
//-- Apr 2013 - merged several updates from Sept 2012 (on git)                 --//                                                                            --//
//--                                                                           --//
//-- Apr 18, 2013- added herring HCR to iscam, see harvest_rate() and          --//
//--               avail_harvest()                                             --//
//--                                                                           --//
// ----------------------------------------------------------------------------- //

//\\//\\//\\ Modified Oct 21, 2016 by Samuel D N Johnson //\\//\\//\\
//\\//\\//\\ Changed to include a data input fix_m_devs
//\\//\\//\\ which enables log_m_devs to be fixed to a 
//\\//\\//\\ supplied value, thereby allowing for testing
//\\//\\//\\ the model's bias when the estimation of M is
//\\//\\//\\ perfect.
...
DATA_SECTION
	...
	//moved these ofstream commands to DATA_SECTION to prevent files from
	//  being improperly left open (JSC: 24Jul13)
	...
	init_number a;
	init_number b;
	//ah and gh are now deprecated
	init_number ah;				// age at 50% maturity
	init_number gh;				// slope of maturity ogive
	//
	init_vector mat(sage,nage)  // the model now requires that the maturity ogive be calculated externally and input here
	...
	// Added by SDNJ to allow for fixing m_devs to the true value
	// in a sim-est procedure.
	init_int n_m_class;
	init_matrix fix_m_devs(1,n_m_class,syr+1,nyr);
	...
	// 15-> switch for selectivity normalisation (0=mean,1=max,2=none)
	// FIXME: document cntrl(14).
	init_vector cntrl(1,15);
	int verbose;
	
	init_ivector m_dev_phzs(1,n_m_class);
	!! cout << "m_dev_phzs = " << m_dev_phzs << endl;
	init_vector mAges(1,n_m_class);
	
	//operational control point
	init_number cutoff_fraction; // proportion of prefishery biomass to unfished biomass used to calculate cutoff
	init_number target_hr;
	init_number fixed_cutoff;
...
PARAMETER_SECTION
	...
	init_bounded_vector_vector log_m_nodesV(1,n_m_class,1,n_m_devs,-5.,5.,m_dev_phzs);
	matrix log_m_nodes(1,n_m_class,1,n_m_devs);

	//init_bounded_vector log_m_dev(syr+1,nyr,-5.0,5.0,m_dev_phz);
	...
	vector m(1,n_m_class);					//initial natural mortality rate
	...
	matrix log_m_devs(1,n_m_class,syr+1,nyr); // log deviations in natural mortality
...
FUNCTION double harvest_rate(double prefishery_bt, double unfished_b, double cutoff_fraction, double target_hr)  //hr_cutoff, unfished_bio
  {
...
FUNCTION double avail_harvest(double prefishery_bt, double unfished_b, double cutoff_fraction, double target_hr)
  {
