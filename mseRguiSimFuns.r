...
# 22. Only calc ref pts on initialization, or when OM button touched, or 
#     CtlFile button touched.  No need for constant recalcs.
#
#------------------------------------------------------------------------------#
...
#                                                                              #
# REQUIRES: PBSmodelling, RODBC, ref, snow                                     #
#                                                                              #
# References:                                                                  #
#                                                                              #
...
.mseRsetupSim <- function(win)
{
  ...
  require( ref )                     # For passing arguments by reference.
  ...
  if ( goMenu )
  {
    ...
    if ( win == "mseRguiSim" )
    {
      ...
      guiChanges$scenarioLabel <- ctlList$gui$scenarioLabel
      guiChanges$mpLabel       <- ctlList$gui$mpLabel
      ...
      # ARK *** Need to install "ref" and change this call.
      
      #refPts <- calcRefPoints( ctlList$opMod, rpList=list( FALL=TRUE, x=40 ) )
      tmp <- ctlList$opMod
      refPts <- deref( calcRefPoints( as.ref( tmp ) ) )
      assign( "refPtList", refPts, pos=1 )
      ...
      .doGuiSimPlots( ctlPars, refPtList )
      focusWin( win )
    }
  }
  else
  ...
  else     # Sub-menus are active...
  {
    ...
    else if ( win=="mseRguiOpMod" )
    {
      ...
      if ( validPars )
      {
        ...
        tmp    <- ctlList$opMod
        refPts <- calcRefPoints( as.ref( tmp ) )
...
.validDataPars <- function( tMP=50, nT=100 )
{
  ...
  # Ages constraints: 1<t1Ages<t2Ages<nT  AND 0<k1Ages<(nT-t1) & 0<k2Ages<(nT-t2)  
  ...
  #if ( is.na(k1Ages) || (k1Ages < 1) || (!intVal(k1Ages)) )
  #{
  #  cat( "\nMSG (.validDataPars) Frequency of Period 1 ages must be integer and k1Ages >= 1.\n" )
  #  guiChanges$k1Ages <- NA
  #  isValid <- FALSE
  #}

  #if ( is.na(k2Ages) || (k2Ages < 1) || (!intVal(k2Ages)) )
  #{
  #  cat( "\nMSG (.validDataPars) Frequency of Period 2 ages must be integer and k2Ages >= 1.\n" )
  ...
  guiChanges$useAges <- gsub( " ","",useAges )
  guiChanges$t1Ages  <- gsub( " ","",t1Ages )
  guiChanges$t2Ages  <- gsub( " ","",t2Ages )
  guiChanges$k1Ages  <- gsub( " ","",k1Ages )
  guiChanges$k2Ages  <- gsub( " ","",k2Ages )
...
.validHcrPars <- function()
{
  ...
  if ( is.na(lowerBoundMult) || (lowerBoundMult < 0.0) || (lowerBoundMult > upperBoundMult) )
  {
    cat( "\nMSG (.validHcrPars) Lower bound multiplier must be 0 < lowerBoundMult < upperBoundMult < 1.\n" )
...
# GUI outputs:  Clears invalid fields
#               Corrects correctable fields (if any)
.validMethodPars <- function( tMP=19, nT=45 )
{
  intVal <- function( x )
  {
    # Is the value of x an integer?  There must be a better way...
...
.validOpModPars <- function()
{
  ...
  if (!.validHistoryFile( historyType, tMP ))
  {
    ...
    guiChanges$historyType <- "simAuto" 
  }  
  ...
  {
    cat( "\nMSG (.validOpModPars) Age at 50% selectivity must be aSel50 > 1.\n" )
    cat( "\nMSG (.validOpModPars) Age at 95% selectivity must be greater than age at 50% selectivity.\n")
...
.validSimPars <- function()
{
  ...
  if ( is.element( simName, simsDone ) )
  {
    ...
    alarm()
    if ( getYes( paste( "Scenario and procedure labels reset to unique values\n\n",
                        "Do you want to enter new labels?" ) ) )
    {
      isValid <- FALSE
    }                        
...
.setGuiDataState <- function()
{
  ...
  # Set options for age data timing.
...
.setGuiHcrState <- function( methodId=.CAAMOD )
{
  ...
  setWidgetState( varname="statusBase" ,  radiovalue="statusBaseData", state="disabled" )
  setWidgetState( varname="statusSource", radiovalue="statusSrceData", state="disabled" )
  
  # Turn off all but variable F.
  setWidgetState( varname="hcrType", radiovalue="constantF",   state="disabled" )
  setWidgetState( varname="hcrType", radiovalue="declineRisk", state="disabled" )
...
.setGuiOpModState <- function()
{
  ...
  if ( historyType == "omFile" )
  {
    ...
    if ( !file.exists( hstFile ) )
    {
      ...
      alarm()
      guiChanges$simHistoryType <- "simAuto"
    }
...
.getSimStateList <- function( ctlList )
{
  ...
  stateList <- list( methodName=.METHODLAB[ctlList$mp$assess$methodId],
                     ...
                     simAge=simAge, simSurvey=simSurvey )
  stateList
...
.setGuiSimState <- function( stateList )
{
  ...
  # Disable OM submenu.
  setWidgetState( varname="simOM", state="disabled", winname="mseRguiSim" )
...
# Returns:       NULL (invisibly).
# Source:        A.R. Kronlund
.doGuiSimPlots <- function( ctlPars, refPts )
{
  ...
  # ARK (14-Aug-13) When blobs are rebuilt they will have a refpts limb, and
  # maybe a schedules limb, to get rid of blob$pars.  In the interim, use
  # blob$pars.  Yes but in guiSim, there is no blob yet.  So maybe guiSim should
  # have a temporary object that is refpts and schedules, that simply is discarded
  # once the sim is run?  I need to reduce the number of times refpts are calculated.
  ...
  checkedF         <- c( chkFmsy, chkF0, chkF01, chkF40, chkFmax, chkFcra )
  names( checkedF ) <- c( "Fmsy",  "F0",  "F01",  "F40",  "Fmax",  "Fcra"  )
  checkedU         <- checkedF
  names( checkedU ) <- c( "Umsy",  "U0",  "U01", "U40",   "Umax",  "Ucra" )  
  ...
  else if ( simPlotType=="simLenAtAge" )
  {
    ...
    xLim <- NULL
    if ( simSetXaxis )
      xLim <- c( simMinAge,simMaxAge )
    yLim <- NULL
    if ( simSetYaxis )
      yLim <- c( simMinLen,simMaxLen )
    .plotLenAtAge( schedules, gfx=list( annotate=simAnnotate, xLim=xLim, yLim=yLim ) )
  }
  ...
  else if ( simPlotType=="simMatAtAge" )
  {
    ...
    if ( simSetYaxis )
      yLim <- c( 0.0,1.0 )
    .plotMatAtAge( schedules, gfx=list( annotate=simAnnotate, xLim=xLim, yLim=yLim ) )
  }
  ...
  else if ( simPlotType=="simNumAtAge" )
  {
    ...
    .plotNumAtAge( refPts, gfx=list( annotate=simAnnotate, doLegend=simLegend,
                   xLim=xLim, yLim=yLim ) )
  }
  ...
  else if ( simPlotType=="simWgtAtAge" )
  {
    ...
    yLim <- NULL
    if ( simSetYaxis )
      yLim <- c( simMinWgt,simMaxWgt )
    .plotWgtAtAge( schedules, gfx=list( xLim=xLim, yLim=yLim ) )
  }
  ...
  else if ( simPlotType=="simWgtLen" )
  {
    ...
    xLim <- NULL
    if ( simSetXaxis )
      xLim <- c( simMinLen,simMaxLen )
    yLim <- NULL
    if ( simSetYaxis )
      yLim <- c( minWgt,maxWgt )
    .plotWgtLen( schedules, gfx=list( xLim=xLim, yLim=yLim ) )
  }
  ...
  else if ( simPlotType=="simLifeHist" )
  {
    ...
    refPts$numAgeYr1 <- refPts$numAgeYr1/1.e6
    .plotNumAtAge( refPts, gfx=list( annotate=simAnnotate, doLegend=simLegend ) )
    ...
    if ( simSetYaxis )
      yLim <- c( simMinWgt,simMaxWgt )
    schedules$Wal <- schedules$Wal*1.e6
    .plotWgtAtAge( schedules, gfx=list( xLim=xLim, yLim=yLim ) )
  ...
  else if ( simPlotType=="simRecSSB" | simPlotType=="simRecSSB2" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )     
  
    .plotRecSSB( refPts,gfx=list( annotate=simAnnotate,checked=checkedF, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
  ...
  else if ( simPlotType=="simSsbF" )
  {
    ...
    .plotSsbF( refPts,gfx=list( annotate=simAnnotate,checked=checkedF, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
  ...
  else if ( simPlotType=="simSsbRF" )
  {
    ...
    .plotSsbPerRecF( refPts,gfx=list( annotate=simAnnotate,checked=checkedF, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
  ...
  else if ( simPlotType=="simYieldF" )
  {
    ...
    .plotYieldF( refPts,gfx=list( annotate=simAnnotate,checked=checkedF,doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
                 xLim=c(simMinF,simMaxF),yLim=c(simMinYield,simMaxYield) ) )
  }
  
  else if ( simPlotType=="simYieldSSB" | simPlotType=="simYieldSSB2" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )     
  
    .plotYieldSSB( refPts,gfx=list( annotate=simAnnotate,checked=checkedF, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
  ...
  else if ( simPlotType=="simYprF" )
  {
    ...
    .plotYprF( refPts,gfx=list( annotate=simAnnotate,checked=checkedF, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
                 xLim=c(simMinF,simMaxF),yLim=c(simMinYPR,simMaxYPR) ) )
  }
  
  # Combined reference point plot.
  else if ( simPlotType=="simRefPtsF" )
  {
    if ( simAuto )
    {
      myOma <- .OMA
    ...
    .plotRefPointsF( refPts, gfx=list( annotate=simAnnotate, checked=checkedF, doLegend=simLegend,
                      setXaxis=simSetXaxis, setYaxis=simSetYaxis ) )
  }
  
  else if ( simPlotType=="simSsbU" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )   
  
    .plotSsbU( refPts,gfx=list( annotate=simAnnotate,checked=checkedU, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
                 xLim=c(simMinF,simMaxF),yLim=c(simMinSSB,simMaxSSB) ) )
  }
  
  else if ( simPlotType=="simSsbRU" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )     
  
    .plotSsbPerRecU( refPts,gfx=list( annotate=simAnnotate,checked=checkedU, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
                 xLim=c(simMinF,simMaxF),yLim=c(simMinSSBR,simMaxSSBR) ) )
  }
  
  else if ( simPlotType=="simYieldU" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )     
  
    .plotYieldU( refPts,gfx=list( annotate=simAnnotate,checked=checkedU, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
                 xLim=c(simMinF,simMaxF),yLim=c(simMinYield,simMaxYield) ) )
  }
  
  else if ( simPlotType=="simYprU" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )     
  
    .plotYprU( refPts,gfx=list( annotate=simAnnotate,checked=checkedU, doLegend=simLegend,
                 setXaxis=simSetXaxis, setYaxis=simSetYaxis,
                 xLim=c(simMinF,simMaxF),yLim=c(simMinYPR,simMaxYPR) ) )
  }
  
  # Combined reference point plot.
  else if ( simPlotType=="simRefPtsU" )
  {
    if ( simAuto )
    {
      myOma <- .OMA
    ...
    .plotRefPointsU( refPts, gfx=list( annotate=simAnnotate, checked=checkedU, doLegend=simLegend,
                      setXaxis=simSetXaxis, setYaxis=simSetYaxis ) )
  }
  ...
  else if ( simPlotType=="simSlg" )
  {
    ...
    .plotSlg( schedules, gfx=list(autolayout=simAuto, annotate=simAnnotate,
                     bygear=simGears, doLegend=simLegend, xLim=xLim, yLim=NULL ) )
  }
                           
  # Selectivity by age, length and gear.
  else if ( simPlotType=="simSalg" )
  {
    if ( simAuto )
    {
      mfRow <- .getGearRowCol( ctlList$opMod$nGear )
    ...
    .plotSalg( schedules, gfx=list(autolayout=simAuto, annotate=simAnnotate,
                       doLegend=simLegend, xLim=xLim, yLim=NULL ) )
  }

  # Proportion discarded at length and gear.
  else if ( simPlotType=="simPlg" )
  {
    ...
    .plotPlg( schedules, gfx=list(autolayout=simAuto, annotate=simAnnotate,
                     bygear=simGears, doLegend=simLegend, xLim=xLim, yLim=NULL ) )
  }

  # Proportion discarded at age, length and gear.
  else if ( simPlotType=="simPalg" )
  {
    if ( simAuto )
    {
      mfRow <- .getGearRowCol( ctlList$opMod$nGear )
    ...
    .plotPalg( schedules, gfx=list(autolayout=simAuto, annotate=simAnnotate,
                       doLegend=simLegend, xLim=xLim, yLim=NULL ) )
  }
  
  # Mortality rates of discards by gear.
  else if ( simPlotType=="simdg" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )  
  
    .plotDisMortRate( schedules, annotate=simAnnotate )
  }

  # Gear impacts.
  else if ( simPlotType=="simYieldGear" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )  
  
    .plotYieldByGear( refPts, gfx=list( annotate=simAnnotate, checked=checkedU ) )
  }

  # Compare scenarios.
  else if ( simPlotType=="simFvsh" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )  
    .plotFvsh( simTracker )
  ...
  else if ( simPlotType=="simYieldVsF" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )
        
    .plotYieldVsF( simTracker, refGear=3, gfx=list( annotate=simAnnotate, checked=checkedU) )
  }
  ...
  else if ( simPlotType == "simHCR" )
  {
    ...
    if ( simSetXaxis )
      xLim <- c( simMinSSB,simMaxSSB )
    else
      xLim <- NULL  
      
    if ( simSetYaxis )
      yLim <- c( simMinF,simMaxF )
    else
      yLim <- NULL
  
    .plotMpHCR( ctlList, refPts, gfx=list( annotate=simAnnotate, doLegend=simLegend,
                xLim=xLim, yLim=yLim ) )
  }

  else if ( simPlotType == "simOptHCR" )
  {
    if ( simAuto )
      par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )  
    
    if ( ctlList$mp$hcr$hcrType=="variableF" )
      .plotHCRvariableF( ctlList, refPts,
                         gfx=list( annotate=simAnnotate, doLegend=simLegend ) )
    ...
    if ( ctlList$mp$hcr$hcrType=="constantF" )
      .plotHCRconstantF( ctlList, gfx=list( annotate=simAnnotate, doLegend=simLegend ) )
  ...
  else if ( simPlotType == "simCtg" )
  {
    if ( simAuto )
    {
      if ( simGears )
      {
        ...
        myMar <- .MAR
        myMar[2] <- 4
        par( oma=myOma, mar=myMar, mfrow=mfRow )
      }
      else
        par( oma=.OMA, mar=.MAR, mfrow=c(1,1) )
    }
    
    xLim <- c( 1,ctlList$opMod$tMP )
    if ( simSetXaxis )
      xLim <- c( simMinYr,simMaxYr )
    yLim <- NULL
    if ( simSetYaxis )
      yLim <- c( simMinCat,simMaxCat )
  
    # Make a fake blob$om$Ctg
    
    tmp <- ctlList$mp$data$inputCatch$landCatchMatrix
    tmpCtg <- array( NA, dim=c(1,ctlList$opMod$nT,ctlList$opMod$nGear) )
    iGear <- 0
    for ( j in 3:ncol(tmp) )
    {
      iGear <- iGear + 1
      tmpCtg[1,1:ctlList$opMod$tMP-1,iGear] <- tmp[,j]
    }
    
    tmpBlob <- list()
    tmpBlob$ctlList <- ctlList
    tmpBlob$om$Ctg  <- tmpCtg
    .plotCtg( tmpBlob, gfx=list( annotate=simAnnotate, bygears=simGears,
                                 doLegend=simLegend, showProj=FALSE,
                                 xLim=xLim, yLim=yLim, useYears=simYrs ) )
  }
  
  else if ( simPlotType == "simDtg" )
  {
    if ( simAuto )
    {
      if ( simGears )
    ...
    tmp <- ctlList$mp$data$inputCatch$relCatchMatrix
    tmpDtg <- array( NA, dim=c(1,ctlList$opMod$nT,ctlList$opMod$nGear) )
    iGear <- 0
    for ( j in 3:ncol(tmp) )
    {
      iGear <- iGear + 1
      tmpDtg[1,1:ctlList$opMod$tMP-1,iGear] <- tmp[,j]
    }
    
    tmpBlob <- list()
    tmpBlob$ctlList <- ctlList
    tmpBlob$om$Dtg  <- tmpDtg
    .plotDtg( tmpBlob, gfx=list( annotate=simAnnotate, bygears=simGears,
                                 doLegend=simLegend, showProj=FALSE,
                                 xLim=xLim, yLim=yLim, useYears=simYrs ) )
  }      
  ...
  else if ( simPlotType == "simIndexPars" )
  {
    ...
    .plotIndexSeries( ctlList, gfx=list( annotate=simAnnotate, bygears=simGears,
                               doLegend=simLegend, showProj=FALSE,
                               xLim=xLim, yLim=yLim, useYears=simYrs ) )
  }
  ...
  else if ( simPlotType == "simDesign" )
  {
    ...
    .plotDesign( ctlList, gfx=list( annotate=simAnnotate, doLegend=simLegend,
      xLim=xLim, yLim=yLim, years=simYrs ) ) 
  }
  
  else if ( simPlotType=="simPriors" )
  {
    if ( simAuto )
    ...
    .plotPriors( ctlList, gfx=list( annotate=simAnnotate, doLegend=simLegend,
                 xLim=xLim, yLim=yLim ) )
  }  
...
.formatRefPts <- function( refPts, digits=NULL )
{
  ...
  if ( !is.null(digits) )
  {
    ...
    result$U0        <- round( refPts$U0,          digits=4 )
    result$U01       <- round( refPts$U01,         digits=4 )
    result$U40       <- round( refPts$U40,         digits=4 )
    result$Umsy      <- round( refPts$Umsy,        digits=4 )
    result$Ucra      <- round( refPts$Ucra,        digits=4 )
    result$Umax      <- round( refPts$Umax,        digits=4 )
    result$legalHRFmsy <- round( refPts$legalHRFmsy, digits=4 )
  }
  else
  {
    ...
    result$U0        <- refPts$U0
    result$U01       <- refPts$U01
    result$U40       <- refPts$U40
    result$Umsy      <- refPts$Umsy
    result$Ucra      <- refPts$Ucra
    result$Umax      <- refPts$Umax
    result$legalHRFmsy <- refPts$legalHRFmsy
  }
