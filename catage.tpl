// Catch-age model from http://otter-rsch.com/examples.htm
// See Section 1.20 (p. 1-32) in the ADMB manual included with PBSadmb.

// Notice the declaration "likeprof_number pred_B" in the PARAMETER_SECTION.
// The command "catage -lprof" generates the file "pred_B.plt" with
// likelihood profile estimates for this parameter (the predicted biomass).
...
PROCEDURE_SECTION
  ...
  if ( mceval_phase() )
  {
    // Write a line with the sampled values
    cout << avg_F << " " << predicted_N(2,nages) << " " << ratio_N(2,nages) << endl; 
  }
