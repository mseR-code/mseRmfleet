...
.saveOptionsToScript <- function( genOpts, prjOpts )
{
  ...
  varDone <- rep( FALSE, nrow(prjOpts) )
  for ( i in 1:nrow( prjOpts ) )
  {
    if ( varDone[i]==FALSE )
    {
      # Check for duplicate occurences of a variable name, if so build vectors.
    
      dupes <- is.element( prjOpts$varName, prjOpts$varName[i] )
      if ( sum(dupes)> 1 )
      {
        ...
        nDupes <- sum( dupes )
        labels <- c( "BG","COL","FG","LTY","LWD","PCH","CEX","LAB","UNIT" )
        varList <- c( "bg","col","fg","lty","lwd","pch","cex","label","units" )
        
        idx <- order( prjOpts[ dupes,"index" ] )
        
        for ( j in 1:length(varList) )
        {
          vals <- prjOpts[ dupes,varList[j] ]
          
          # Now put in order of index.
          vals <- vals[ idx ]
          
          cat( file=optFile, paste( prefix,labels[j]," <- c(", sep="" ), append=TRUE )
          for ( k in 1:nDupes )
          {
