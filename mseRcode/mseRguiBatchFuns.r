...
# A1. There is a base simCtlFile file in "batch" folder containing all the     #
#     parameters that are required for mseR.  Any parameter value that occurs  #
#     in the batch job control file OVERRIDES the value contained in the base  #
#     parameter file.                                                          #
#                                                                              #
...
# (b) Base File  - the mseR guiSim() base file, same format as simCtlFile.txt. #
#                                                                              #
# guiBatch allows the user to Select or View two output files:                 #
# (c) Design File - lists the control files, scenario and management procedure #
...
.createBatchDesign <- function( ctlList, basePars, prefix="sim" )
{
  ...
  for ( i in 1:nScenario )
  {
    ...
    for ( j in 1:nMP )
    {
      ...
      # Set default scenarioLabel and mpLabel values in case not supplied.
      scenarioName[iBatch] <- paste( "scenario",i,sep="" )
      mpName[iBatch]       <- paste( "mp",j,sep="" )
  ...
  for ( i in 1:nSims )
  {
    if ( file.exists( batchParFile[i] ) )
    {
      ...
      cat( "\nWARNING (.runBatchJob) simCtlFile.txt overwritten by ",
           batchParFile[i],"\n" )
      file.copy( file.path( .PRJFLD, .DEFBATFLD, batchParFile[i] ),
...
.subBatch <- function()
{
  ...
  if ( valid )
  {
    ...
    {
      val <- .setFile( batBasePars, c( ".t","txt files" ) )
      if ( !is.null(val) )
    ...
    if ( act=="batMake" )
    {
      ...
      # Load the base simulation control file.
...
.guiBatchSetup <- function( win )
{
  ...
  # ARK (12-Oct-13) Change to force batch folder to hold base file (a file like
  # simCtlFile.txt).  Then, if user alters simCtlFile, the batch job is not
  # changed on the next Make call.
  
  # 1. Look for simCtlFile.txt in the batch folder.
  # 2. If not found, copy the one from the project folder.
  # 3. Set the 

  # (1) If required, initialize .BASEFILE to default simCtlFile filename in working directory.
  if ( !exists( ".BASEFILE" ) )
  {
   .BASEFILE <- file.path( .PRJFLD,.DEFBATFLD,.DEFBASEFILE )
    assign( ".BASEFILE", .BASEFILE, pos=1 )
  }
  
  # If .BASEFILE initialized, but file does not exist, then copy simCtlFile from
  # the working directory.
  if ( !file.exists( .BASEFILE ) )
  {
    ctlFile <- file.path( wkDir, basename(.CTLFILE) )
    file.copy( ctlFile, .BASEFILE )
    if ( .WHINE > 0 )
  ...
  if ( goMenu )
  {
    ...
    guiChanges$batBasePars <- .BASEFILE
    guiChanges$batDesFile  <- file.path( .PRJFLD,.DEFBATFLD, batDesFile )
    guiChanges$batJobFile  <- file.path( .PRJFLD,.DEFBATFLD, batJobFile )
    guiChanges$batPrjFld   <- .PRJFLD
