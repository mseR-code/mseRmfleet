  ...
  if ( exists( ".PRJFLD" ) )
  {
    ...
    if ( .PRJFLD != tmpPrjFld )
    {
      ...
      if ( getYes( paste( "mseRinitProject: Reset project folder to ", tmpPrjFld ) ) )
      {
        ...
        cat( "\nMSG( .mseRinitProject) Set batch base file to ", .BASEFILE, "\n" )        
      }
    }
  }
  else
                       ...
                       xPos=NULL, yPos=NULL,... )
{
  ...
  #txt <- paste( "    MP: ",procedure,sep="" )
  #txt <- paste("      ",procedure,sep="" )
  txt <- paste(scenario,":",procedure,sep="" )
...
{
  # Input: obj - the estPars dataframe from blob$mp$assess$estPars.
  ...
  # For now, if estPars is NULL, then there are no estimated reference points
  # i.e., the methodId must have been .KALMAN or .MOVINGAVG.
  ...
  {  
    ...
    {
      # Find replicate ID numbers where EstRefPt is "DefaultUnRel".
      
      # Get vector of replicate identifiers where reference points "DefaultUnRel".
      # Find replicate ID numbers where EstRefPt is "DefaultUnRel", since estPars
      # has more than one row for each replicate as progressive years are estimated
      # from tMP to nT.
    ...
    if ( keepFailConv==FALSE )
    {
      ...
      # since estPars has more than one row for each replicate as progressive years
      # are estimated from tMP to nT.
  
      failConv  <- unique( obj$iRep[ obj$iExit!=1 ] )
      nFailConv <- sum( failConv )
    }
