# parBatchFuns.R
# parBatchRun wrapped into a function for 
# the multiBatchScript.R script
...
# runParBatch()
# Machinery to set up multiple copies
# of the wd and run mseR in parallel
# while using ADMB AMs
runParBatch <- function( nSims, nCores )
{
  batchFiles  <- 1:nSims
  maxCores    <- nCores
  ...
  # Load batch design
  desPath <- file.path(getwd(),"mseRproject","batch","batchDesign.txt")
  ...
  nCores  <- min(nBatchFiles,detectCores()-1,maxCores)
  ...
  for (idx in (1:length(batchFiles)))
  {
    ...
    x <- try(file.copy(from=source,to=destination,recursive=TRUE))
