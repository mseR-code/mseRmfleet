...
function(fn)
{
	...
	for(i in 1:nv)
	{
		...
		if(irr-ir>2) 
		{
			dum=try(as.matrix(read.table( fn, skip=ir, nrow=irr-ir-1, 
																fill=TRUE,sep = "")),silent = TRUE )
			SCAN <- FALSE
			if( class(dum) == "try-error" ) SCAN <- TRUE
			else if( class(dum) == "matrix" & (!is.numeric(dum[nrow(dum),] ) ) ) SCAN <- TRUE
			if( vnam[i] %in% c("pit","it","epsilon")) SCAN <- TRUE
			if( SCAN )
			{
				maxLength <- 1
				for(ii in 0:(irr-ir-2) )
				{
					# This really slows things down...
					x <- as.double(scan(fn,skip=ir+ii,nlines=1,quiet=TRUE,what=""))
					maxLength <- max(maxLength, length(x))
				}
				dum=as.matrix(read.table( fn, skip=ir, nrow=irr-ir-1, 
																	fill=TRUE,sep = "",col.names = 1:maxLength))	
			}
		}
